...
fn pebble_header_fixup(b: *std.Build, pebble_include_path: []const u8) []const u8 {
    var pebble_header = std.Io.Dir.cwd().readFileAlloc(b.graph.io, b.pathJoin(&.{ pebble_include_path, "pebble.h" }), b.allocator, .limited(1 << 20)) catch @panic("OOM");
...
fn pebble_linker_script_template(b: *std.Build, pebble_linker_script_template_path: []const u8, platform: PebblePlatform) []const u8 {
    ...
    const linker_script_aligned = std.mem.replaceOwned(u8, b.allocator, linker_script_with_got, "KEEP(*(.pbl_header))", "KEEP(*(.pbl_header))\n        . = ALIGN(4);") catch @panic("OOM");

    // Template max app memory size
    return std.mem.replaceOwned(u8, b.allocator, linker_script_aligned, "@MAX_APP_MEMORY_SIZE@", b.fmt("0x{X}", .{PEBBLE_PLATFORMS.getAssertContains(platform).MAX_APP_MEMORY_SIZE})) catch @panic("OOM");
}
...
pub fn addPebbleApplication(b: *std.Build, options: PebbleApplicationOptions) void {
    ...
    const pebble_sdk_path = b.option([]const u8, "pebble_sdk_path", "Pebble SDK path (default ~/.pebble-sdk/SDKs/current/)") orelse path: {
        if (b.graph.environ_map.get("HOME")) |home_path| {
    ...
    for (options.pebble.targetPlatforms) |platform| {
        ...
        translate_pebble.defineCMacroRaw("wint_t=unsigned int");
        ...
        exe.root_module.addObject(appinfo_object);
    ...
    package_pbw_step.addArg(std.fmt.allocPrint(b.allocator, "{d}", .{pebble_app_artifacts.items.len}) catch @panic("OOM"));
